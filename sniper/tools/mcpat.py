...
def main(jobid, resultsdir, outputfile, powertype = 'dynamic', config = None, no_graph = False, partial = None, print_stack = True, return_data = False, dtlb_type = 'dtlb', l1d_type = 'l1d'):
  ...
  power, nuca_at_level = edit_XML(stats, results['results'], results['config'], dtlb_type, l1d_type)
...
def edit_XML(statsobj, stats, cfg, dtlb_type, l1d_type):
...
#---------------------------
  ...
  template=readTemplate(ncores, num_l2s, private_l2s, num_l3s, technology_node, dtlb_type, l1d_type)
    ...
    #for j in range(ncores):
      ...
      def get_vdd(component):
        ...
        if domain == 'core':
          ...
          raise ValueError('Unknown DVFS domain %s' % domain)
      ##### RBERA CHANGES #####
      def get_size(component):
          return int(sniper_config.get_config(cfg, component+'/size', core))
      def get_pwc_size(component):
          return int(sniper_config.get_config(cfg, component+'_size', core))
      #########################
      ...
      ###### RBERA CHANGES ######
      uperm_cacheAssociativity = power2up(long(sniper_config.get_config_default(cfg, 'perf_model/utopia/pcache/assoc', 0, core)))
      uperm_cacheBlockSize = 64 # hardcoding 
      uperm_cacheSize= long(sniper_config.get_config_default(cfg, 'perf_model/utopia/pcache/size', 0, core))
      utag_cacheAssociativity = power2up(long(sniper_config.get_config_default(cfg, 'perf_model/utopia/tagcache/assoc', 0, core)))
      utag_cacheBlockSize = 64 # hardcoding 
      utag_cacheSize= long(sniper_config.get_config_default(cfg, 'perf_model/utopia/tagcache/size', 0, core))
      ###########################
      ...
      if template[i][1]:
        ...
        if len(template[i][1]) == 1:
          ...
          elif template[i][1][0]=="itlb_size":
            template[i][0] = template[i][0] % get_size('perf_model/itlb')
          elif template[i][1][0]=="dtlb_size":
            template[i][0] = template[i][0] % get_size('perf_model/dtlb')
          ####### RBERA CHANGES #######
          elif template[i][1][0]=="stlb_size":
            template[i][0] = template[i][0] % get_size('perf_model/stlb')
          elif template[i][1][0]=="pwc_l2_size":
            template[i][0] = template[i][0] % get_pwc_size('perf_model/ptw/pwc/l2')
          elif template[i][1][0]=="pwc_l3_size":
            template[i][0] = template[i][0] % get_pwc_size('perf_model/ptw/pwc/l3')
          elif template[i][1][0]=="pwc_l4_size":
            template[i][0] = template[i][0] % get_pwc_size('perf_model/ptw/pwc/l4')
          elif template[i][1][0]=="ulb_size":
            template[i][0] = template[i][0] % get_size('perf_model/utopia/ulb')
          #############################
          else:
          ...
          if template[i][1][0]=="total_cycles":
            ...
            if float((long(stats['L1-D.tloads'][core])+long(stats['L1-D.tstores'][core]))/max_system_cycles) <= 1:
              template[i][0] = template[i][0] % float((long(stats['L1-D.tloads'][core])+long(stats['L1-D.tstores'][core]))/max_system_cycles)
            else:
              template[i][0] = template[i][0] % 1
          elif template[i][1][0]=="MemManU.I.duty_cycle":
            if float((long(stats['L1-I.tloads'][core])+long(stats['L1-I.tstores'][core]))/max_system_cycles) <= 1:
              template[i][0] = template[i][0] % float((long(stats['L1-I.tloads'][core])+long(stats['L1-I.tstores'][core]))/max_system_cycles)
            else:
              template[i][0] = template[i][0] % 1
            ...
            template[i][0] = template[i][0] % long(stats['L1-D.tloads'][core])
          elif template[i][1][0]=="stores":
            template[i][0] = template[i][0] % long(stats['L1-D.tstores'][core])
            ...
            template[i][0] = template[i][0] % int(stats['L1-I.tloads'][core] + stats['L1-I.tstores'][core])
            ...
            template[i][0] = template[i][0] % int(stats['L1-I.tloads'][core])
          elif template[i][1][0]=="icache.read_misses":
            template[i][0] = template[i][0] % int(stats['L1-I.tload-misses'][core])
          elif template[i][1][0]=="dtlb.total_accesses":        #dtlb equals dcache reads and writes
            template[i][0] = template[i][0] % int(stats['L1-D.tloads'][core] + stats['L1-D.tstores'][core])
          elif template[i][1][0]=="dtlb.total_misses":
            template[i][0] = template[i][0] % int(stats['L1-D.tload-misses'][core] + stats['L1-D.tstore-misses'][core])
          ######## RBERA CHANGES ########
          elif template[i][1][0]=="stlb.total_accesses":
            template[i][0] = template[i][0] % int(stats['stlb.access'][core])
          elif template[i][1][0]=="stlb.total_misses":
            template[i][0] = template[i][0] % int(stats['stlb.miss'][core])
          elif template[i][1][0]=="pwc_l2.total_accesses":
            template[i][0] = template[i][0] % int(stats['pwc_L2.access'][core])
          elif template[i][1][0]=="pwc_l2.total_misses":
            template[i][0] = template[i][0] % int(stats['pwc_L2.miss'][core])
          elif template[i][1][0]=="pwc_l3.total_accesses":
            template[i][0] = template[i][0] % int(stats['pwc_L3.access'][core])
          elif template[i][1][0]=="pwc_l3.total_misses":
            template[i][0] = template[i][0] % int(stats['pwc_L3.miss'][core])
          elif template[i][1][0]=="pwc_l4.total_accesses":
            template[i][0] = template[i][0] % int(stats['pwc_L4.access'][core])
          elif template[i][1][0]=="pwc_l4.total_misses":
            template[i][0] = template[i][0] % int(stats['pwc_L4.miss'][core])
          elif template[i][1][0]=="ulb.total_accesses":
            template[i][0] = template[i][0] % int(stats['ulb.access'][core])
          elif template[i][1][0]=="ulb.total_misses":
            template[i][0] = template[i][0] % int(stats['ulb.miss'][core])
          ###############################
          elif template[i][1][0]=="dcache.read_accesses":
            template[i][0] = template[i][0] % int(stats['L1-D.tloads'][core])
          elif template[i][1][0]=="dcache.read_misses":
            template[i][0] = template[i][0] % int(stats['L1-D.tload-misses'][core])
          ######## RBERA CHANGES ########
          elif template[i][1][0]=="uperm.read_accesses":
            template[i][0] = template[i][0] % int(stats['uperm.access'][core])
          elif template[i][1][0]=="uperm.read_misses":
            template[i][0] = template[i][0] % int(stats['uperm.miss'][core])
          elif template[i][1][0]=="utag.read_accesses":
            template[i][0] = template[i][0] % int(stats['utag.access'][core])
          elif template[i][1][0]=="utag.read_misses":
            template[i][0] = template[i][0] % int(stats['utag.miss'][core])
          ###############################
          elif template[i][1][0]=="dcache.write_accesses":
            template[i][0] = template[i][0] % int(stats['L1-D.tstores'][core])
          elif template[i][1][0]=="dcache.write_misses":
            template[i][0] = template[i][0] % int(stats['L1-D.tstore-misses'][core])
          elif template[i][1][0]=="function_calls":
          ...
          # L2 caches
            ...
            template[i][0] = template[i][0] % sum([ stats['L2.tloads'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.write_accesses":
            template[i][0] = template[i][0] % sum([ stats['L2.tstores'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.read_misses":
            template[i][0] = template[i][0] % sum([ stats['L2.tload-misses'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2.write_misses":
            template[i][0] = template[i][0] % sum([ stats['L2.tstore-misses'][c] for c in cores_l2s ])
          elif template[i][1][0]=="L2_duty_cycle":
            template[i][0] = template[i][0] % min(1,(sum([ stats['L2.tloads'][c] + stats['L2.tstores'][c] for c in cores_l2s ]) / float(max_system_cycles)))
          # L3 caches
          elif template[i][1][0]=="L3.read_accesses":
            template[i][0] = template[i][0] % ('L3.tloads' in stats and sum([ stats['L3.tloads'][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3.write_accesses":
            template[i][0] = template[i][0] % ('L3.tstores' in stats and sum([ stats['L3.tstores'][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3.read_misses":
            template[i][0] = template[i][0] % ('L3.tload-misses' in stats and sum([ stats['L3.tload-misses'][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3.write_misses":
            template[i][0] = template[i][0] % ('L3.tstore-misses' in stats and sum([ stats['L3.tstore-misses'][c] for c in cores_l3s ]) or 0)
          elif template[i][1][0]=="L3_duty_cycle":
            template[i][0] = template[i][0] % min(1,('L3.tloads' in stats and sum([ stats['L3.tloads'][c] + stats['L3.tstores'][c] for c in cores_l3s ]) / float(max_system_cycles) or 0))
          else:
          ...
          elif template[i][1][0]=="uperm_cfg":
            upermconf=[]
            upermconf.append(int(uperm_cacheSize)*1024)
            upermconf.append(uperm_cacheBlockSize)
            upermconf.append(uperm_cacheAssociativity)
            upermconf.append(1)            #banks
            upermconf.append(10)
            upermconf.append(10*1) # hardcoded
            upermconf.append(0) # unused?
            upermconf.append(0) # 1 for writeback
            template[i][0] = template[i][0] % tuple(upermconf)
          elif template[i][1][0]=="utag_cfg":
            utagconf=[]
            utagconf.append(int(utag_cacheSize)*1024)
            utagconf.append(utag_cacheBlockSize)
            utagconf.append(utag_cacheAssociativity)
            utagconf.append(1)            #banks
            utagconf.append(10)
            utagconf.append(10*1) # hardcoded
            utagconf.append(0) # unused?
            utagconf.append(0) # 1 for writeback
            template[i][0] = template[i][0] % tuple(utagconf)
  return template, nuca_at_level
#----------
def readTemplate(ncores, num_l2s, private_l2s, num_l3s, technology_node, dtlb_type, l1d_type):
  Count = 0
  template=[]
...
#-------------------------------------------------------------------------------------------------------------------------
    ...
    template.append(["\t\t\t\t<param name=\"number_entries\" value=\"%i\"/>",["itlb_size","cfg",iCount]])                #not in graphite (whether correct)
...
#       template.append(["\t\t\t\t<param name=\"buffer_sizes\" value=\"%i, 16, %i, %i\"/>",["icache_buffer","comb"]]) #mcpat will crash for some different sizes of 2nd parameter
    ...
    template.append(["\t\t\t</component>",""])
    ##### RBERA CHANGES #####
    ##### DTLB aliasing with stlb, PWC, and ULB ######
    print("dtlb_type value {}".format(dtlb_type))
    if (dtlb_type == 'dtlb'):
        template.append(["\t\t\t<component id=\"system.core%i.dtlb\" name=\"dtlb\">"%iCount,""])
        template.append(["\t\t\t\t<param name=\"number_entries\" value=\"%i\"/>",["dtlb_size","cfg",iCount]])
        ...
        template.append(["\t\t\t<component id=\"system.core%i.dtlb\" name=\"dtlb\">"%iCount,""])
        template.append(["\t\t\t\t<param name=\"number_entries\" value=\"%i\"/>",["stlb_size","cfg",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",["stlb.total_accesses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_misses\" value=\"%i\"/>",["stlb.total_misses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
        template.append(["\t\t\t</component>",""])
    elif (dtlb_type == 'pwc_l2'):
        template.append(["\t\t\t<component id=\"system.core%i.dtlb\" name=\"dtlb\">"%iCount,""])
        template.append(["\t\t\t\t<param name=\"number_entries\" value=\"%i\"/>",["pwc_l2_size","cfg",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",["pwc_l2.total_accesses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_misses\" value=\"%i\"/>",["pwc_l2.total_misses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
        template.append(["\t\t\t</component>",""])
    elif (dtlb_type == 'pwc_l3'):
        template.append(["\t\t\t<component id=\"system.core%i.dtlb\" name=\"dtlb\">"%iCount,""])
        template.append(["\t\t\t\t<param name=\"number_entries\" value=\"%i\"/>",["pwc_l3_size","cfg",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",["pwc_l3.total_accesses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_misses\" value=\"%i\"/>",["pwc_l3.total_misses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
        template.append(["\t\t\t</component>",""])
    elif (dtlb_type == 'pwc_l4'):
        template.append(["\t\t\t<component id=\"system.core%i.dtlb\" name=\"dtlb\">"%iCount,""])
        template.append(["\t\t\t\t<param name=\"number_entries\" value=\"%i\"/>",["pwc_l4_size","cfg",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",["pwc_l4.total_accesses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_misses\" value=\"%i\"/>",["pwc_l4.total_misses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
        template.append(["\t\t\t</component>",""])
    elif (dtlb_type == 'ulb'):
        template.append(["\t\t\t<component id=\"system.core%i.dtlb\" name=\"dtlb\">"%iCount,""])
        template.append(["\t\t\t\t<param name=\"number_entries\" value=\"%i\"/>",["ulb_size","cfg",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_accesses\" value=\"%i\"/>",["ulb.total_accesses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"total_misses\" value=\"%i\"/>",["ulb.total_misses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
        template.append(["\t\t\t</component>",""])
    ##### L1-D aliasing with Utopia PF and TAR cache ###### 
    print("l1d_type value {}".format(l1d_type))
    if (l1d_type == 'l1d'):
        ...
        template.append(["\t\t\t<component id=\"system.core%i.dcache\" name=\"dcache\">"%iCount,""])
        template.append(["\t\t\t\t<!-- all the buffer related are optional -->",""])
        template.append(["\t\t\t\t<param name=\"dcache_config\" value=\"%i,%i,%i,%i,%i,%i, %i, %i\"/>",["uperm_cfg","comb",iCount]])
        #template.append(["\t\t\t\t<param name=\"dcache_config\" value=\"32768,32,8,1, 4,6, 32,1 \"/>",""])
        template.append(["\t\t\t\t<param name=\"buffer_sizes\" value=\"16, 16, 16, 16\"/>",""]) #mcpat will crash for some different sizes of 2nd parameter
        template.append(["\t\t\t\t<!-- cache controller buffer sizes: miss_buffer_size(MSHR),fill_buffer_size,prefetch_buffer_size,wb_buffer_size-->",""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",["uperm.read_accesses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"write_accesses\" value=\"0\"/>",""]) # all accesses are reads
        template.append(["\t\t\t\t<stat name=\"read_misses\" value=\"%i\"/>",["uperm.read_misses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"write_misses\" value=\"0\"/>",""]) # all misses are reads
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
        template.append(["\t\t\t</component>",""])
    elif (l1d_type == 'tar'):
        template.append(["\t\t\t<component id=\"system.core%i.dcache\" name=\"dcache\">"%iCount,""])
        template.append(["\t\t\t\t<!-- all the buffer related are optional -->",""])
        template.append(["\t\t\t\t<param name=\"dcache_config\" value=\"%i,%i,%i,%i,%i,%i, %i, %i\"/>",["utag_cfg","comb",iCount]])
        #template.append(["\t\t\t\t<param name=\"dcache_config\" value=\"32768,32,8,1, 4,6, 32,1 \"/>",""])
        template.append(["\t\t\t\t<param name=\"buffer_sizes\" value=\"16, 16, 16, 16\"/>",""]) #mcpat will crash for some different sizes of 2nd parameter
        template.append(["\t\t\t\t<!-- cache controller buffer sizes: miss_buffer_size(MSHR),fill_buffer_size,prefetch_buffer_size,wb_buffer_size-->",""])
        template.append(["\t\t\t\t<stat name=\"read_accesses\" value=\"%i\"/>",["utag.read_accesses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"write_accesses\" value=\"0\"/>",""]) # all accesses are reads
        template.append(["\t\t\t\t<stat name=\"read_misses\" value=\"%i\"/>",["utag.read_misses","stat",iCount]])
        template.append(["\t\t\t\t<stat name=\"write_misses\" value=\"0\"/>",""]) # all misses are reads
        template.append(["\t\t\t\t<stat name=\"conflicts\" value=\"0\"/>",""])
        template.append(["\t\t\t</component>",""])
    #########################
...
if __name__ == '__main__':
  ...
  try:
    opts, args = getopt.getopt(sys.argv[1:], "hj:t:c:d:o:", [ 'no-graph', 'no-text', 'partial=', 'dtlb-type=', 'l1d-type='])
  ...
  main(jobid = jobid, resultsdir = resultsdir, powertype = powertype, config = config, outputfile = outputfile, no_graph = no_graph, print_stack = not no_text, partial = partial, dtlb_type = dtlb_type, l1d_type = l1d_type)
