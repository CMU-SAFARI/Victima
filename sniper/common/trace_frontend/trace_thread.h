...
class TraceThread : public Runnable
{
   private:
      ...
      { return ((TraceThread*)arg)->handleForkFunc();}
        static void  __handleMallocEnterFunc(void* arg, uint64_t pointer,  uint64_t size)
      { return ((TraceThread*)arg)->handleMallocEnterFunc(pointer,size);}
      static void  __handleCallocEnterFunc(void* arg, uint64_t pointer,  uint64_t size)
      { return ((TraceThread*)arg)->handleCallocEnterFunc(pointer,size);}
      static void  __handleReallocEnterFunc(void* arg, uint64_t init_pointer, uint64_t pointer,  uint64_t size)
      { return ((TraceThread*)arg)->handleReallocEnterFunc(init_pointer,pointer,size);}
      static void  __handleFreeEnterFunc(void* arg, uint64_t pointer)
      { return ((TraceThread*)arg)->handleFreeEnterFunc(pointer);}
      ...
      void handleMallocEnterFunc(uint64_t pointer, uint64_t size );
      void handleCallocEnterFunc(uint64_t pointer, uint64_t size);
      void handleReallocEnterFunc(uint64_t init_pointer, uint64_t pointer, uint64_t size);
      void handleFreeEnterFunc(uint64_t pointer);
