   ...
   , m_stopped(false)
{
   ...
   m_trace.setHandleMallocEnterFunc(TraceThread::__handleMallocEnterFunc,this);
   m_trace.setHandleCallocEnterFunc(TraceThread::__handleCallocEnterFunc,this);
   m_trace.setHandleReallocEnterFunc(TraceThread::__handleReallocEnterFunc,this);
   m_trace.setHandleFreeEnterFunc(TraceThread::__handleFreeEnterFunc,this);
   ...
   {
      printf("VAAAAAAAAAAAAAAAAAAAAAAU FAAAAIL\n");
   }
...
void TraceThread::handleFreeEnterFunc(uint64_t pointer){

	//std::cout << "[Sniper] FREE_ENTER " << " Pointer: "  << pointer << std::endl ;

	//Sim()->getAllocationManager()->handleFree(pointer,m_thread->getCore()->getId());

}



void TraceThread::handleMallocEnterFunc(uint64_t pointer, uint64_t size){

   //std::cout << "[Sniper] MALLOC_ENTER " << " Pointer: "  << pointer << " Size: " << size ;
	//Sim()->getAllocationManager()->handleMalloc(pointer,size, m_thread->getCore()->getId());
}

void TraceThread::handleCallocEnterFunc(uint64_t pointer, uint64_t size){

	//std::cout << "[Sniper] CALLOC_ENTER from Sniper side \n";
	//Sim()->getAllocationManager()->handleCalloc(pointer,size, m_thread->getCore()->getId());

}

void TraceThread::handleReallocEnterFunc(uint64_t init_pointer, uint64_t pointer, uint64_t size){

	//std::cout << "[Sniper] REALLOC_ENTER from Sniper side \n";
	//Sim()->getAllocationManager()->handleRealloc(init_pointer,pointer,size, m_thread->getCore()->getId());
...
void TraceThread::handleRoutineChangeFunc(Sift::RoutineOpType event, uint64_t eip, uint64_t esp, uint64_t callEip)
{
   switch(event)
   {
      ...
      case Sift::RoutineExit:
         ...
         break;
      case Sift::MallocEnter:
         m_thread->getRoutineTracer()->routineEnter(eip, esp, callEip);
         m_thread->getRoutineTracer()->mallocEnter(eip, esp, callEip);
         break;
      case Sift::MallocExit:
         m_thread->getRoutineTracer()->routineExit(eip, esp);
         m_thread->getRoutineTracer()->mallocExit(eip, esp);
         break;
      ...
      if (!dec_inst.is_nop())
      {
         ...
         for(uint32_t mem_idx = 0; mem_idx < Sim()->getDecoder()->num_memory_operands(&dec_inst); ++mem_idx)
         {
	       ...
	       */
	       ...
	       UInt64 va = mem_address; /* pa == va @konkanello */

               if (is_atomic_update)
                  core->logMemoryHit(false, Core::WRITE, va, Core::MEM_MODELED_COUNT, va2pa(inst.sinst->addr));
               else
                  core->accessMemory(
